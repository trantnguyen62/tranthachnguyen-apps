    ...
    topics: [
        {
            ...
            quiz: [
                { question: 'What is the main purpose of a Dockerfile?', options: ['To run containers', 'To define instructions for building an image', 'To manage container networks', 'To store container data'], correct: 1, explanation: 'A Dockerfile is a text file containing ordered instructions (FROM, RUN, COPY, etc.) that Docker reads to automatically build an image. Each instruction creates a new layer in the image.' },
                { question: 'Which command is used to build a Docker image?', options: ['docker run', 'docker build', 'docker create', 'docker start'], correct: 1, explanation: 'docker build reads the Dockerfile and builds an image. Use -t to tag the image (e.g., docker build -t myapp:1.0 .) and specify the build context as the final argument.' },
                { question: 'What does Docker Compose primarily help with?', options: ['Building single containers', 'Managing multi-container applications', 'Storing images', 'Monitoring containers'], correct: 1, explanation: 'Docker Compose uses a docker-compose.yml file to define and run multi-container applications as a single unit. It manages service dependencies, shared networks, and volumes with simple commands like docker compose up/down.' },
                { question: 'Where is data stored when using Docker volumes?', options: ['Inside the container', 'In the image layers', 'Outside the container filesystem', 'In Docker Hub'], correct: 2, explanation: 'Docker volumes store data in a Docker-managed area on the host (typically /var/lib/docker/volumes/). Because they live outside the container filesystem, data persists after the container is stopped or removed.' },
                { question: 'What is a multi-stage build used for?', options: ['Running multiple containers', 'Creating smaller final images', 'Managing multiple registries', 'Building on multiple platforms'], correct: 1, explanation: 'Multi-stage builds use multiple FROM instructions in one Dockerfile. Heavy build tools and dependencies stay in early stages; only compiled artifacts are copied into the lean final image, dramatically reducing its size and attack surface.' },
                { question: 'Which is Docker\'s native orchestration tool?', options: ['Kubernetes', 'Docker Swarm', 'Mesos', 'Nomad'], correct: 1, explanation: 'Docker Swarm is Docker\'s built-in clustering and orchestration solution. It turns a pool of Docker hosts into a single virtual host and supports rolling updates, service scaling, and encrypted overlay networks natively.' },
                { question: 'What does the -d flag do in docker run?', options: ['Deletes the container after exit', 'Runs in detached/background mode', 'Enables debug mode', 'Downloads the image'], correct: 1, explanation: 'The -d (detached) flag starts the container in the background and returns the container ID. Without -d the container runs in the foreground, attaching stdout/stderr to your terminal.' },
                { question: 'What is Docker Buildx used for?', options: ['Building Docker Compose files', 'Multi-platform builds', 'Container networking', 'Image scanning'], correct: 1, explanation: 'Docker Buildx extends the build command with BuildKit features, including building images for multiple CPU architectures (linux/amd64, linux/arm64, etc.) simultaneously using --platform. This is essential for supporting Apple Silicon and ARM-based cloud instances.' },
                { question: 'Which network driver enables cross-host communication?', options: ['bridge', 'host', 'overlay', 'none'], correct: 2, explanation: 'The overlay network driver creates a distributed network that spans multiple Docker hosts in a Swarm cluster, enabling containers on different machines to communicate securely without exposing ports to the host network.' },
                { question: 'How are Docker Secrets stored?', options: ['Plain text files', 'Environment variables', 'Encrypted at rest', 'In Dockerfile'], correct: 2, explanation: 'Docker Secrets are encrypted at rest and in transit. They are stored in the Raft log of the Swarm manager and only transmitted to containers that need them, mounted as files at /run/secrets/ — never exposed as environment variables.' },
                { question: 'What does the --rm flag do in docker run?', options: ['Removes the image after use', 'Automatically removes container when it exits', 'Removes all other containers', 'Resets memory usage'], correct: 1, explanation: 'The --rm flag tells Docker to automatically delete the container\'s writable layer once it exits. This is useful for one-off tasks and keeps your system clean without needing manual docker rm cleanup.' },
                { question: 'What is the purpose of .dockerignore?', options: ['Ignore containers', 'Exclude files from build context', 'Ignore network issues', 'Skip health checks'], correct: 1, explanation: 'A .dockerignore file lists paths to exclude from the build context sent to the Docker daemon. Excluding .git, node_modules, and test files speeds up builds and prevents accidentally copying secrets into the image.' },
                { question: 'Which instruction sets environment variables that persist in running containers?', options: ['ARG', 'ENV', 'SET', 'VAR'], correct: 1, explanation: 'ENV sets environment variables that are baked into the image and available at both build time and runtime. ARG is a build-only variable that is not present in the final image or running containers.' },
                { question: 'What is the difference between COPY and ADD?', options: ['No difference', 'ADD can extract archives and download URLs', 'COPY is slower', 'ADD is deprecated'], correct: 1, explanation: 'COPY simply copies files from the build context into the image. ADD does everything COPY does but also automatically extracts local tar archives and can fetch files from remote URLs. Best practice is to use COPY unless you specifically need ADD\'s extra features.' },
                { question: 'What is containerd?', options: ['A container name', 'A container registry', 'An industry-standard container runtime', 'A Docker command'], correct: 2, explanation: 'containerd is a CNCF-graduated container runtime that manages the full container lifecycle (pulling images, creating/starting/stopping containers). Docker uses containerd under the hood, as does Kubernetes via the CRI interface.' },
                { question: 'What does docker system prune do?', options: ['Updates Docker', 'Removes unused data', 'Prunes container processes', 'Backs up system'], correct: 1, explanation: 'docker system prune removes all stopped containers, dangling images, unused networks, and optionally unused volumes. Add -a to also remove all images not referenced by a running container. Useful for reclaiming disk space.' },
                { question: 'What is a bind mount?', options: ['Type of network', 'Mounts host file/directory into container', 'Encryption method', 'Volume driver'], correct: 1, explanation: 'A bind mount maps a specific path on the host filesystem directly into the container (e.g., -v /host/path:/container/path). Changes are immediately visible on both sides, making bind mounts ideal for development but riskier in production than named volumes.' },
                { question: 'What is the scratch image?', options: ['Damaged image', 'An explicitly empty image for minimal builds', 'Testing image', 'Debug image'], correct: 1, explanation: 'scratch is a special Docker image that is completely empty — no OS, no shell, no packages. It\'s used as the base in multi-stage builds for statically compiled binaries (Go, Rust), producing the smallest possible image with minimal attack surface.' },
                { question: 'What is Docker rootless mode?', options: ['Running without root privileges', 'Running without network', 'Minimal container mode', 'Read-only mode'], correct: 0, explanation: 'Rootless mode allows the Docker daemon and containers to run without root privileges on the host. This significantly reduces the risk of container escape vulnerabilities by ensuring that even a compromised container cannot gain host root access.' },
                { question: 'What does docker logs -f do?', options: ['Filters logs', 'Follows/streams logs in real-time', 'Formats logs', 'Finds specific logs'], correct: 1, explanation: 'The -f (follow) flag streams new log output in real-time, similar to tail -f. Without -f, docker logs prints all existing output and exits. Combine with --since or --tail to limit output volume.' },
                { question: 'What is an image digest?', options: ['Image description', 'SHA256 hash uniquely identifying an image', 'Image size', 'Build number'], correct: 1, explanation: 'A digest is the SHA256 hash of an image\'s manifest, providing an immutable identifier. Unlike tags (which can be reused), pulling an image by digest (e.g., myimage@sha256:abc123) guarantees you get the exact same image every time.' },
                { question: 'What is DinD (Docker in Docker)?', options: ['Docker installation', 'Running Docker inside a Docker container', 'Docker network mode', 'Docker daemon type'], correct: 1, explanation: 'DinD runs a full Docker daemon inside a container. It\'s commonly used in CI/CD pipelines to build Docker images. However, it requires --privileged mode and carries security risks; Docker-outside-of-Docker (mounting the host socket) or Kaniko are often safer alternatives.' },
                { question: 'What is Alpine Linux commonly used for in Docker?', options: ['Database hosting', 'Minimal base image (~5MB)', 'Web servers only', 'Windows containers'], correct: 1, explanation: 'Alpine Linux is a security-focused, minimal Linux distribution weighing around 5MB. It\'s widely used as a Docker base image to create small, fast-to-pull images. It uses musl libc and BusyBox instead of glibc and GNU utils.' },
                { question: 'What does docker pause do?', options: ['Stops containers', 'Suspends container processes using cgroups freezer', 'Pauses image download', 'Delays builds'], correct: 1, explanation: 'docker pause suspends all processes in a container using the Linux cgroups freezer subsystem. The container remains in memory but no CPU time is allocated. docker unpause resumes execution. Useful for temporarily halting work without losing state.' },
                { question: 'What is the Docker socket path?', options: ['/etc/docker', '/var/run/docker.sock', '/home/docker', '/tmp/docker'], correct: 1, explanation: 'The Docker daemon listens on a Unix socket at /var/run/docker.sock by default. Mounting this socket into a container grants it full control of the Docker daemon — treat it like root access and never expose it to untrusted containers.' },
                { question: 'What is BuildKit?', options: ['Build tool kit', 'Modern build subsystem with improved caching', 'Docker extension', 'Container kit'], correct: 1, explanation: 'BuildKit is Docker\'s modern build backend that enables parallel stage execution, advanced cache mounts (--mount=type=cache), SSH forwarding, and secret mounts during builds. Enable it with DOCKER_BUILDKIT=1 or use docker buildx build.' },
                { question: 'What does EXPOSE instruction do in Dockerfile?', options: ['Opens firewall ports', 'Documents which ports container listens on', 'Publishes ports automatically', 'Exposes secrets'], correct: 1, explanation: 'EXPOSE is purely documentation — it tells users which ports the containerized application listens on, but does NOT publish them. To actually publish ports to the host, you must use -p HOST:CONTAINER with docker run.' },
                { question: 'What is a tmpfs mount?', options: ['Permanent storage', 'Temporary filesystem stored in memory', 'Tarball filesystem', 'Template filesystem'], correct: 1, explanation: 'A tmpfs mount creates a temporary filesystem in the host\'s RAM. Data is not persisted to disk and disappears when the container stops. It\'s useful for storing sensitive data (tokens, keys) that should never be written to disk.' },
                { question: 'What command shows running processes in a container?', options: ['docker ps', 'docker top', 'docker proc', 'docker list'], correct: 1, explanation: 'docker top CONTAINER shows the running processes inside a container, similar to the Unix top/ps command. docker ps lists running containers themselves, not the processes within them.' },
                { question: 'How do you limit container memory?', options: ['--mem-limit', '--memory', '--max-mem', '--ram'], correct: 1, explanation: 'The --memory flag (or -m) sets a hard memory limit (e.g., docker run --memory 512m). When the limit is reached, the container\'s processes are subject to the OOM killer. Also use --memory-swap to control swap usage.' },
                { question: 'What is Docker Content Trust?', options: ['License agreement', 'Digital signature verification for images', 'Backup system', 'Access control'], correct: 1, explanation: 'Docker Content Trust (DCT) uses Notary to cryptographically sign and verify image tags. When DOCKER_CONTENT_TRUST=1 is set, Docker refuses to pull or run unsigned images, protecting against tampered or malicious images.' },
                { question: 'What is the OCI?', options: ['Online Container Index', 'Open Container Initiative - industry standards', 'Original Container Image', 'Official Container Installer'], correct: 1, explanation: 'The Open Container Initiative (OCI) is a Linux Foundation project that publishes open industry standards for container formats and runtimes (image-spec and runtime-spec). This ensures containers built with one tool can run on any OCI-compliant runtime.' },
                { question: 'What command shows image layer history?', options: ['docker layers', 'docker history', 'docker show', 'docker info'], correct: 1, explanation: 'docker history IMAGE shows all layers in an image along with the command that created each layer, its size, and creation time. Use --no-trunc to see full commands. Helpful for understanding image size and debugging large layers.' },
                { question: 'What is the default network driver?', options: ['host', 'bridge', 'overlay', 'macvlan'], correct: 1, explanation: 'The bridge driver is the default. Containers on the same bridge network can communicate via IP; they are isolated from the host network unless ports are explicitly published with -p. User-defined bridges also provide automatic DNS resolution by container name.' },
                { question: 'What does docker cp do?', options: ['Copies containers', 'Copies files between container and host', 'Copies images', 'Copies networks'], correct: 1, explanation: 'docker cp copies files or directories between a container and the local filesystem. It works on both running and stopped containers. Syntax: docker cp CONTAINER:SRC_PATH DEST_PATH or docker cp SRC_PATH CONTAINER:DEST_PATH.' },
                { question: 'What is a distroless image?', options: ['Image without distribution', 'Image with only app and runtime, no shell', 'Uncompressed image', 'Raw image'], correct: 1, explanation: 'Distroless images (by Google) contain only the application and its runtime dependencies — no package manager, shell, or other OS utilities. This drastically reduces the attack surface since attackers cannot spawn a shell or install tools if they gain access.' },
                { question: 'What restart policy restarts unless manually stopped?', options: ['always', 'on-failure', 'unless-stopped', 'never'], correct: 2, explanation: 'unless-stopped restarts the container automatically (including after daemon restart) unless it was explicitly stopped with docker stop. The always policy is similar but will restart even manually stopped containers when the Docker daemon restarts.' },
                { question: 'What does docker diff show?', options: ['Image differences', 'Container filesystem changes', 'Version differences', 'Config differences'], correct: 1, explanation: 'docker diff CONTAINER lists all files/directories that have been added (A), changed (C), or deleted (D) in the container\'s writable layer compared to the image it was started from. Useful for debugging unexpected filesystem modifications.' },
                { question: 'What is docker-compose down -v used for?', options: ['Downloads volumes', 'Stops and removes containers, networks, and volumes', 'Verifies down', 'Verbose down'], correct: 1, explanation: 'docker compose down stops and removes all containers and networks defined in the Compose file. The -v flag also removes named volumes declared in the volumes section, giving you a completely clean slate. Without -v, volumes are preserved.' },
                { question: 'What is Podman?', options: ['Docker plugin', 'Daemonless container engine compatible with Docker', 'Pod manager only', 'Docker GUI'], correct: 1, explanation: 'Podman is a daemonless, rootless container engine from Red Hat that is CLI-compatible with Docker (alias docker=podman). Without a central daemon, each container runs as a child of the user\'s process, improving security and enabling pod-level grouping similar to Kubernetes.' }
            ],
            codebase: [
                {
...
exit 0`
        ...
        },
        {
            ...
            quiz: [
                { question: 'What is the smallest deployable unit in Kubernetes?', options: ['Container', 'Pod', 'Deployment', 'Node'], correct: 1, explanation: 'A Pod is the smallest deployable unit in Kubernetes. It can contain one or more containers that share the same network namespace (same IP) and can share storage volumes. Containers in a Pod always run on the same node.' },
                { question: 'Which resource provides a stable network endpoint for Pods?', options: ['Deployment', 'ConfigMap', 'Service', 'Ingress'], correct: 2, explanation: 'A Service provides a stable virtual IP (ClusterIP) and DNS name that load-balances traffic across a set of Pods matched by label selector. Because Pod IPs change on restart, Services decouple consumers from the ephemeral Pod lifecycle.' },
                { question: 'What is the purpose of a Namespace?', options: ['Store secrets', 'Isolate resources within a cluster', 'Manage external traffic', 'Store configuration'], correct: 1, explanation: 'Namespaces provide a virtual partition within a Kubernetes cluster. Teams, environments (dev/staging/prod), or applications can be isolated with separate resource quotas, RBAC policies, and network policies.' },
                { question: 'Which Kubernetes object is used to store sensitive data?', options: ['ConfigMap', 'Secret', 'Volume', 'Service'], correct: 1, explanation: 'Secrets store sensitive data like passwords, tokens, and certificates. Unlike ConfigMaps, Secrets are base64-encoded and can be encrypted at rest. They can be mounted as files or exposed as environment variables in Pods.' },
                { question: 'What does a DaemonSet ensure?', options: ['All pods have unique names', 'A pod runs on all/some nodes', 'External access to services', 'Data persistence'], correct: 1, explanation: 'A DaemonSet ensures exactly one Pod runs on every node (or on nodes matching a selector). It\'s ideal for node-level infrastructure agents like log collectors (Fluentd), monitoring agents (Prometheus node-exporter), or CNI plugins.' },
                { question: 'What is Helm used for in Kubernetes?', options: ['Container runtime', 'Package management', 'Network policies', 'Log collection'], correct: 1, explanation: 'Helm is the Kubernetes package manager. Charts are reusable templates that define a complete application stack. Helm handles install, upgrade, rollback, and dependency management, and is the de-facto standard for distributing complex Kubernetes applications.' },
                { question: 'Which command-line tool is used to interact with Kubernetes?', options: ['docker', 'kubectl', 'helm', 'kubeadm'], correct: 1, explanation: 'kubectl is the official CLI for interacting with any Kubernetes cluster. It communicates with the kube-apiserver using credentials from your kubeconfig file. Common commands: get, describe, apply, delete, logs, exec, port-forward.' },
                { question: 'What workload is best for stateful applications?', options: ['Deployment', 'ReplicaSet', 'StatefulSet', 'DaemonSet'], correct: 2, explanation: 'StatefulSets give each Pod a stable, predictable hostname (e.g., db-0, db-1) and dedicated PersistentVolumes that are retained across restarts. These guarantees are essential for databases and distributed systems like Kafka or ZooKeeper.' },
                { question: 'What is etcd used for in Kubernetes?', options: ['Container runtime', 'Key-value store for cluster data', 'Load balancing', 'Pod scheduling'], correct: 1, explanation: 'etcd is a distributed, strongly consistent key-value store that serves as the single source of truth for all cluster state — every object, configuration, and secret. Loss of etcd data means loss of the cluster; regular backups are critical.' },
                { question: 'What does the kubelet do?', options: ['Schedules pods', 'Runs containers in pods', 'Manages API requests', 'Stores configuration'], correct: 1, explanation: 'The kubelet runs on every worker node. It watches the API server for Pods assigned to its node, instructs the container runtime (containerd) to start/stop containers, and reports back pod status and node resource usage.' },
                { question: 'What is a PersistentVolumeClaim?', options: ['Storage request by user', 'Pod definition', 'Network policy', 'Container image'], correct: 0, explanation: 'A PersistentVolumeClaim (PVC) is a user\'s request for storage. It specifies required size and access mode. The cluster binds the PVC to a matching PersistentVolume (PV), abstracting the details of the underlying storage provider from the Pod.' },
                { question: 'Which service type exposes a static port on each node?', options: ['ClusterIP', 'NodePort', 'LoadBalancer', 'ExternalName'], correct: 1, explanation: 'NodePort opens a static port (30000-32767) on every cluster node. Traffic to any node IP on that port is forwarded to the Service. It\'s a simple way to expose services externally but generally LoadBalancer or Ingress is preferred in production.' },
                { question: 'What does HPA stand for?', options: ['High Performance API', 'Horizontal Pod Autoscaler', 'Host Pod Access', 'HTTP Proxy Agent'], correct: 1, explanation: 'The Horizontal Pod Autoscaler automatically scales the number of Pod replicas based on observed CPU/memory utilization or custom metrics. It checks metrics at regular intervals (default 15s) and scales up or down within defined min/max bounds.' },
                { question: 'What is a CronJob used for?', options: ['Continuous deployment', 'Scheduled tasks', 'Persistent storage', 'Network policies'], correct: 1, explanation: 'A CronJob creates Jobs on a time-based schedule using standard cron syntax (e.g., "0 2 * * *" for 2am daily). Each execution creates a new Job object. Common uses: database backups, report generation, cache cleanup.' },
                { question: 'What is an Init Container?', options: ['Container for initialization', 'Main application container', 'Sidecar container', 'Debug container'], correct: 0, explanation: 'Init containers run and complete sequentially before any app container starts. They\'re used for setup tasks like waiting for a database, running DB migrations, fetching secrets, or cloning a git repo into a shared volume.' },
                { question: 'What does RBAC stand for?', options: ['Resource Based Access Control', 'Role Based Access Control', 'Runtime Based Access Control', 'Request Based Access Control'], correct: 1, explanation: 'Role-Based Access Control controls what Kubernetes API operations (get, list, create, delete) a user or service account can perform on which resources. Roles/ClusterRoles define permissions; RoleBindings/ClusterRoleBindings assign them.' },
                { question: 'What is a Taint used for?', options: ['Marking pods', 'Repelling pods from nodes', 'Encrypting data', 'Logging'], correct: 1, explanation: 'Taints are applied to nodes to repel Pods that don\'t have a matching Toleration. This lets you reserve nodes for specific workloads (e.g., GPU nodes for ML jobs) or mark problematic nodes so normal workloads aren\'t scheduled there.' },
                { question: 'What probe checks if a container is ready to serve traffic?', options: ['Liveness', 'Readiness', 'Startup', 'Health'], correct: 1, explanation: 'The readiness probe tells Kubernetes when a container is ready to accept traffic. Until it passes, the Pod is removed from Service endpoints. The liveness probe detects and restarts hung containers. The startup probe handles slow-starting applications.' },
                { question: 'What is a Custom Resource Definition (CRD)?', options: ['Custom pod type', 'API extension mechanism', 'Network resource', 'Storage definition'], correct: 1, explanation: 'CRDs extend the Kubernetes API with new resource types without modifying the core API server code. Once a CRD is installed, you can create custom resources just like built-in ones. Operators heavily rely on CRDs to represent their managed applications.' },
                { question: 'What is the Operator pattern?', options: ['Manual operations', 'Packaging apps with custom controllers', 'Network operations', 'Storage operations'], correct: 1, explanation: 'An Operator packages application-specific operational knowledge into a Kubernetes controller. It uses CRDs to define the desired state and watches/reconciles the actual state, automating complex Day 2 operations like scaling, backups, and upgrades.' },
                { question: 'What does kubectl apply do?', options: ['Deletes resources', 'Creates/updates resources from file', 'Shows logs', 'Lists pods'], correct: 1, explanation: 'kubectl apply -f file.yaml creates the resource if it doesn\'t exist, or patches it if it does, using server-side or client-side merge. Unlike kubectl create, it\'s idempotent and is the standard approach for declarative GitOps workflows.' },
                { question: 'What is a Sidecar container?', options: ['Main container', 'Supporting container in same pod', 'Init container', 'Debug container'], correct: 1, explanation: 'A sidecar runs alongside the main application container in the same Pod, sharing its network and storage. Common examples: Envoy proxy (Istio), log shipper (Fluentd), secrets injector (Vault Agent). Sidecar containers can also have independent lifecycles in newer K8s versions.' },
                { question: 'What is Pod Affinity used for?', options: ['Pod deletion', 'Scheduling based on other pods', 'Pod scaling', 'Pod networking'], correct: 1, explanation: 'Pod Affinity/Anti-affinity rules influence scheduling based on the labels of other Pods. Affinity co-locates Pods (e.g., app and cache on same node for low latency). Anti-affinity spreads replicas across nodes/zones for high availability.' },
                { question: 'What is a Headless Service?', options: ['Service without pods', 'Service without ClusterIP', 'Service without port', 'Service without selector'], correct: 1, explanation: 'A Headless Service (clusterIP: None) does not get a virtual IP. Instead, DNS returns the individual Pod IPs directly. This is essential for StatefulSets where clients need to connect to specific Pods (e.g., a Kafka broker or Cassandra node).' },
                { question: 'What controls voluntary disruption limits?', options: ['ResourceQuota', 'LimitRange', 'Pod Disruption Budget', 'Priority Class'], correct: 2, explanation: 'A PodDisruptionBudget (PDB) limits the number of Pods from a deployment that can be simultaneously unavailable during voluntary disruptions (node drain, rolling update). It ensures minimum availability SLAs during cluster maintenance.' },
                { question: 'What does kubectl drain do?', options: ['Adds pods', 'Safely evicts pods from node', 'Deletes node', 'Creates namespace'], correct: 1, explanation: 'kubectl drain marks a node as unschedulable and evicts all Pods, respecting PodDisruptionBudgets. It\'s used before node maintenance or decommissioning to safely migrate workloads to other nodes without downtime.' },
                { question: 'What is Kustomize used for?', options: ['Container building', 'Configuration customization', 'Monitoring', 'Logging'], correct: 1, explanation: 'Kustomize lets you customize Kubernetes YAML manifests without templating. It uses a kustomization.yaml to define patches, overlays, and resource generators. Built into kubectl (kubectl apply -k), it\'s useful for managing environment-specific configuration differences.' },
                { question: 'What is an Ingress Controller?', options: ['API component', 'Implementation of Ingress resource', 'Storage controller', 'Pod controller'], correct: 1, explanation: 'An Ingress Controller is the software (e.g., ingress-nginx, Traefik, AWS ALB) that watches Ingress resources and configures a proxy/load balancer accordingly. Without an Ingress Controller installed, Ingress resources have no effect.' },
                { question: 'What is the CNI?', options: ['Container Name Interface', 'Container Network Interface', 'Cluster Node Integration', 'Control Network Interface'], correct: 1, explanation: 'The Container Network Interface (CNI) is a standard for configuring container networking. Kubernetes delegates Pod networking to CNI plugins like Calico, Cilium, or Flannel which handle IP assignment, routing, and network policy enforcement.' },
                { question: 'What does kubectl port-forward do?', options: ['Opens firewall', 'Forwards local port to pod', 'Exposes service', 'Creates ingress'], correct: 1, explanation: 'kubectl port-forward creates a temporary tunnel from your local machine to a Pod or Service in the cluster. It\'s useful for debugging (e.g., accessing a database or internal dashboard) without creating a public-facing Service.' },
                { question: 'What is a Service Mesh?', options: ['Network cables', 'Service-to-service communication layer', 'Container mesh', 'Pod mesh'], correct: 1, explanation: 'A service mesh adds a dedicated infrastructure layer for service-to-service communication, providing traffic management, mutual TLS (mTLS), observability (metrics, traces, logs), and security policies — typically via sidecar proxies — without changing application code.' },
                { question: 'What is Istio?', options: ['Container runtime', 'Service mesh', 'Package manager', 'Monitoring tool'], correct: 1, explanation: 'Istio is the most widely used service mesh. It injects an Envoy sidecar proxy into each Pod to intercept all traffic. Features include fine-grained traffic routing (canary deployments, circuit breaking), mTLS, and integration with Prometheus/Grafana/Jaeger.' },
                { question: 'What stores cluster, user, and context info?', options: ['ConfigMap', 'Secret', 'kubeconfig', 'Deployment'], correct: 2, explanation: 'The kubeconfig file (default: ~/.kube/config) stores cluster endpoints, authentication credentials, and named contexts. kubectl uses the current context to determine which cluster/namespace to target. Multiple clusters can be managed by switching contexts.' },
                { question: 'What does kubectl scale do?', options: ['Weighs pods', 'Changes replica count', 'Measures resources', 'Scales nodes'], correct: 1, explanation: 'kubectl scale --replicas=N changes the desired replica count of a Deployment, StatefulSet, or ReplicaSet. For production, prefer HPA for automatic scaling based on load rather than manual scaling.' },
                { question: 'What is the Control Plane?', options: ['Worker nodes', 'Orchestration layer managing cluster', 'Network layer', 'Storage layer'], correct: 1, explanation: 'The Control Plane is the "brain" of Kubernetes. It runs kube-apiserver (the API gateway), etcd (state store), kube-scheduler (Pod placement), and kube-controller-manager (reconciliation loops). It does not run workloads itself.' },
                { question: 'What does kubectl rollout undo do?', options: ['Deletes deployment', 'Rolls back to previous version', 'Pauses rollout', 'Restarts pods'], correct: 1, explanation: 'kubectl rollout undo deployment/NAME rolls back to the previous ReplicaSet revision. Kubernetes stores a configurable history of ReplicaSets. Use --to-revision=N to roll back to a specific version. Check history with kubectl rollout history.' },
                { question: 'What is a ValidatingWebhook?', options: ['Creates resources', 'Validates API requests', 'Updates deployments', 'Deletes pods'], correct: 1, explanation: 'Validating Admission Webhooks intercept API requests after authentication/authorization and can accept or reject them based on custom logic. They\'re used to enforce policies (e.g., require resource limits, disallow privileged containers) before objects are persisted.' },
                { question: 'What provides cluster-wide permissions?', options: ['Role', 'ClusterRole', 'RoleBinding', 'ServiceAccount'], correct: 1, explanation: 'A ClusterRole defines permissions that are not namespaced — it can grant access to cluster-level resources (nodes, PVs) or to resources in all namespaces. A regular Role only applies within one namespace.' },
                { question: 'What does kubectl top show?', options: ['Pod images', 'Resource usage', 'Pod labels', 'Node addresses'], correct: 1, explanation: 'kubectl top pods/nodes shows current CPU and memory utilization. It requires the Metrics Server to be installed in the cluster. Use this to spot resource-hungry Pods and right-size resource requests/limits.' },
                { question: 'What is Gateway API?', options: ['API gateway', 'Next-gen service networking API', 'Admin API', 'Storage API'], correct: 1, explanation: 'Gateway API is the next-generation Kubernetes networking API, designed to replace Ingress with richer semantics. It introduces Gateway, HTTPRoute, and TCPRoute resources supporting multi-team ownership, traffic splitting, and header-based routing natively.' }
            ],
            codebase: [
                {
...
affinity: {}`
        ...
        },
        {
            ...
            quiz: [
                { question: 'What is the main goal of Continuous Integration?', options: ['Deploy to production', 'Frequently merge and test code changes', 'Monitor applications', 'Manage infrastructure'], correct: 1, explanation: 'CI encourages developers to merge code into a shared branch frequently (at least daily). Each merge triggers automated builds and tests, catching integration issues early when they\'re cheapest to fix, rather than discovering them during a big-bang merge.' },
                { question: 'What distinguishes Continuous Deployment from Continuous Delivery?', options: ['More testing', 'Automatic deployment without manual approval', 'Better monitoring', 'Faster builds'], correct: 1, explanation: 'Continuous Delivery ensures every change is deployable and ready for production, but a human approves each release. Continuous Deployment goes further — every passing change is automatically deployed to production with no manual intervention.' },
                { question: 'What is a CI/CD pipeline?', options: ['A type of container', 'Automated processes for build, test, and deploy', 'A monitoring tool', 'A version control system'], correct: 1, explanation: 'A CI/CD pipeline is an automated sequence of stages (compile, unit test, integration test, security scan, staging deploy, production deploy) that code changes pass through. It provides fast feedback, quality assurance, and consistent repeatable releases.' },
                { question: 'Which deployment strategy uses two identical environments?', options: ['Canary', 'Rolling', 'Blue-Green', 'A/B Testing'], correct: 2, explanation: 'Blue-Green deployment maintains two identical production environments. New code is deployed to the inactive environment (e.g., green), tested, then traffic is switched instantly via a load balancer. The old environment (blue) becomes the immediate rollback target.' },
                { question: 'What is a build artifact?', options: ['Source code', 'Test results', 'Output of the build process', 'Configuration file'], correct: 2, explanation: 'A build artifact is the deployable output produced by a CI build — a Docker image, JAR file, binary, or NPM package. Artifacts are versioned and stored in registries (Nexus, Artifactory, ECR). A key principle: build once, deploy the same artifact everywhere.' },
                { question: 'What is a Jenkinsfile?', options: ['Jenkins configuration', 'Pipeline definition file', 'Log file', 'Credential file'], correct: 1, explanation: 'A Jenkinsfile is a text file checked into source control that defines a Jenkins pipeline using Groovy DSL (Declarative or Scripted syntax). Storing the pipeline definition alongside code enables version control, code review, and reuse of pipeline logic.' },
                { question: 'What is GitOps?', options: ['Git operations', 'Git as single source of truth for config', 'GitHub operations', 'Git optimization'], correct: 1, explanation: 'GitOps uses a Git repository as the single source of truth for infrastructure and application configuration. Changes are made via pull requests; an agent (Argo CD, Flux) continuously reconciles the live state with the desired state in Git.' },
                { question: 'What is a canary deployment?', options: ['Full release', 'Gradual rollout to subset of users', 'Emergency release', 'Blue-green switch'], correct: 1, explanation: 'A canary deployment routes a small percentage of traffic (e.g., 5%) to the new version while the majority continues to use the stable version. Metrics are observed, and if healthy, the new version gradually receives more traffic until the rollout is complete.' },
                { question: 'What is ArgoCD primarily used for?', options: ['Container building', 'GitOps CD for Kubernetes', 'Monitoring', 'Testing'], correct: 1, explanation: 'Argo CD is a declarative GitOps CD tool for Kubernetes. It watches a Git repository for Kubernetes manifests and automatically syncs the cluster to match. It provides a UI showing the diff between desired Git state and live cluster state.' },
                { question: 'What triggers a pipeline?', options: ['Manual only', 'Events like push, PR, schedule', 'Only on Fridays', 'Random'], correct: 1, explanation: 'CI/CD pipelines can be triggered by many events: a git push, a pull request, a scheduled cron job, a manual trigger, a webhook from another system, or an upstream pipeline completing. Most platforms support all of these trigger types.' },
                { question: 'What is a runner in GitHub Actions?', options: ['Fast pipeline', 'Server that executes workflows', 'Type of action', 'Workflow step'], correct: 1, explanation: 'A runner is the server that executes GitHub Actions workflow jobs. GitHub provides hosted runners (ubuntu-latest, windows-latest, macos-latest). You can also self-host runners for custom hardware, private networks, or cost savings at scale.' },
                { question: 'What is rolling deployment?', options: ['Rollback', 'Incremental updates replacing old versions', 'Fast deployment', 'Manual deployment'], correct: 1, explanation: 'Rolling deployment replaces instances of the old version gradually, a few at a time, while the rest continue serving traffic. It\'s the default Kubernetes Deployment strategy, avoiding both downtime (unlike recreate) and the resource cost of blue-green.' },
                { question: 'What are feature flags used for?', options: ['Flagging errors', 'Toggling features without deploying', 'Marking files', 'Security alerts'], correct: 1, explanation: 'Feature flags (or feature toggles) let you enable/disable features at runtime without deploying new code. This enables trunk-based development, A/B testing, staged rollouts, and instant kill switches if a new feature causes problems in production.' },
                { question: 'What is pipeline as code?', options: ['Coding pipelines', 'Defining pipelines in version-controlled files', 'Pipeline encryption', 'Code testing'], correct: 1, explanation: 'Pipeline as code stores pipeline definitions (Jenkinsfile, .gitlab-ci.yml, .github/workflows/*.yml) in the same repository as the application code. This brings the same benefits as code: version history, peer review, branching, and reusability.' },
                { question: 'What does code coverage measure?', options: ['Code quality', 'Percentage of code executed by tests', 'Code size', 'Code complexity'], correct: 1, explanation: 'Code coverage measures what percentage of source code lines, branches, and functions are executed by automated tests. High coverage reduces the chance of undetected regressions. Quality gates in CI often enforce a minimum coverage threshold (e.g., 80%).' },
                { question: 'What is static analysis?', options: ['Running tests', 'Analyzing code without executing it', 'Performance testing', 'Load testing'], correct: 1, explanation: 'Static analysis examines source code without executing it to detect bugs, security vulnerabilities, style violations, and code smells. Tools like SonarQube, ESLint, Pylint, and Semgrep run in CI pipelines, catching issues before they reach code review.' },
                { question: 'What is a quality gate?', options: ['Code review', 'Criteria code must meet before proceeding', 'Deployment gate', 'Security check'], correct: 1, explanation: 'A quality gate is a set of conditions (minimum test coverage, zero critical security findings, no new bugs) that must be met before a build can proceed to the next pipeline stage. SonarQube quality gates are a common implementation.' },
                { question: 'What is a rollback?', options: ['Moving forward', 'Reverting to previous version', 'Restarting pipeline', 'Deploying backup'], correct: 1, explanation: 'A rollback reverts a deployed system to a known good previous version when a release causes problems. In Kubernetes this is kubectl rollout undo; in blue-green this is a load balancer switch; in databases it requires careful migration design.' },
                { question: 'What is Tekton?', options: ['CI tool', 'Kubernetes-native CI/CD framework', 'Container tool', 'Monitoring tool'], correct: 1, explanation: 'Tekton is a CNCF project that provides cloud-native CI/CD building blocks as Kubernetes CRDs (Task, Pipeline, PipelineRun). Pipelines run as Pods, benefit from Kubernetes RBAC/secrets, and are reusable across projects. Argo Workflows and Tekton are often compared.' },
                { question: 'What are smoke tests?', options: ['Performance tests', 'Preliminary tests for basic functionality', 'Security tests', 'Load tests'], correct: 1, explanation: 'Smoke tests (also called sanity checks) are a minimal set of tests run immediately after deployment to verify the application starts and its most critical paths work. If smoke tests fail, the deployment is rolled back without running the full test suite.' },
                { question: 'What is immutable infrastructure?', options: ['Infrastructure that changes', 'Infrastructure never modified after deployment', 'Static infrastructure', 'Old infrastructure'], correct: 1, explanation: 'With immutable infrastructure, servers are never patched or updated in place. Instead, a new image is built with the changes and deployed as a replacement. This eliminates configuration drift, improves reproducibility, and makes rollbacks trivial.' },
                { question: 'What is a webhook?', options: ['Web application', 'HTTP callback for event notification', 'Security hook', 'API endpoint'], correct: 1, explanation: 'A webhook sends an HTTP POST request to a configured URL when a specific event occurs (e.g., a git push, a new PR, a build completion). Webhooks are how source control systems trigger CI pipelines in real-time without polling.' },
                { question: 'What is matrix build?', options: ['Mathematical build', 'Running tests across multiple configurations', 'Single build', 'Complex build'], correct: 1, explanation: 'A matrix build runs a job across a matrix of variable combinations (e.g., Node.js 18/20 × ubuntu/windows). All combinations run in parallel, ensuring compatibility across multiple environments without manually duplicating job definitions.' },
                { question: 'What are approval gates?', options: ['Security gates', 'Checkpoints requiring approval before deployment', 'Code gates', 'Test gates'], correct: 1, explanation: 'Approval gates pause a pipeline and wait for a designated person or team to manually approve before proceeding to the next stage (usually production deployment). They are the key difference between Continuous Delivery and Continuous Deployment.' },
                { question: 'What is linting?', options: ['Code cleaning', 'Automated checking for code errors', 'Code compression', 'Code encryption'], correct: 1, explanation: 'Linting is automated static analysis that checks code for style violations, potential bugs, undefined variables, and other issues according to a ruleset. Running a linter in CI prevents style debates in code review and catches simple bugs before testing.' }
            ],
            codebase: [
                {
...
echo "Server setup complete!"`
        ...
        },
        {
            ...
            commands: [
                ...
                { command: 'promtool check config prometheus.yml', description: 'Validate Prometheus configuration file' },
                { command: 'promtool check rules alerts.yml', description: 'Validate Prometheus alerting rules' },
                { command: 'curl -XPOST localhost:9090/-/reload', description: 'Hot-reload Prometheus config without restart' },
                { command: 'curl -s localhost:9090/api/v1/targets | jq .', description: 'List all Prometheus scrape targets and their status' },
                { command: 'curl -s localhost:9090/api/v1/rules | jq .', description: 'List all loaded Prometheus rules via API' },
                { command: 'promtool query instant http://localhost:9090 \'up\'', description: 'Run an instant PromQL query from the CLI' },
                { command: 'node_exporter --web.listen-address=":9100"', description: 'Start Node Exporter on port 9100' },
                { command: 'blackbox_exporter --config.file=blackbox.yml', description: 'Start Blackbox Exporter for endpoint probing (HTTP, DNS, TCP)' },
                { command: 'alertmanager --config.file=alertmanager.yml', description: 'Start Alertmanager with config' },
                { command: 'amtool alert query', description: 'List active alerts in Alertmanager' },
                { command: 'amtool silence add alertname=HighCPUUsage --duration=2h', description: 'Silence a Prometheus alert for 2 hours' },
                { command: 'grafana-server', description: 'Start Grafana server' },
                { command: 'grafana-cli plugins install grafana-piechart-panel', description: 'Install a Grafana plugin' },
                { command: 'logcli query \'{app="myapp"}\' --limit=100', description: 'Query Loki logs with LogQL' },
                { command: 'logcli labels', description: 'List all available Loki label names' },
                { command: 'logcli query \'{app="myapp"} |= "error"\' --since=1h', description: 'Search Loki for error lines in the last hour' },
                { command: 'journalctl -u service-name -f', description: 'Follow systemd service logs in real time' },
                { command: 'journalctl --since "1 hour ago" -p err', description: 'Show error-level journal entries from the last hour' },
                { command: 'tail -f /var/log/syslog', description: 'Follow system logs' },
                { command: 'opentelemetry-collector --config otel-config.yaml', description: 'Start the OpenTelemetry Collector with a config file' },
                { command: 'kubectl top nodes', description: 'View CPU and memory usage for all nodes' },
                { command: 'kubectl top pods -n <namespace>', description: 'View CPU and memory usage for pods in a namespace' },
                { command: 'kubectl logs -n monitoring prometheus-0 -f', description: 'Follow Prometheus pod logs in the monitoring namespace' }
            ],
            quiz: [
                { question: 'What is Prometheus primarily used for?', options: ['Log management', 'Monitoring and alerting', 'Container orchestration', 'Configuration management'], correct: 1, explanation: 'Prometheus is an open-source time-series database and monitoring system from the CNCF. It scrapes metrics from HTTP endpoints, stores them locally, and evaluates alerting rules. It\'s the de-facto standard for Kubernetes monitoring.' },
                { question: 'What does Grafana help you create?', options: ['Containers', 'Alerts', 'Visualization dashboards', 'Logs'], correct: 2, explanation: 'Grafana is an open-source visualization platform. It connects to data sources (Prometheus, Loki, Elasticsearch, CloudWatch) and lets you build dashboards with graphs, tables, and alerts. It\'s the standard UI layer for the Prometheus/Loki observability stack.' },
                { question: 'What does ELK stand for?', options: ['Elasticsearch, Logstash, Kibana', 'Elastic, Lambda, Kubernetes', 'Event, Log, Key', 'External Load Keeper'], correct: 0, explanation: 'ELK is Elasticsearch (search/storage), Logstash (ingest/transform), and Kibana (visualization). Filebeat has largely replaced Logstash as the lightweight log shipper, making EFK (with Filebeat) more common. Now often called the Elastic Stack.' },
                { question: 'What is an SLO?', options: ['Service Level Output', 'System Log Observer', 'Server Load Optimizer', 'Service Level Objective'], correct: 3, explanation: 'A Service Level Objective is a specific, measurable target for service reliability (e.g., 99.9% of requests succeed in < 300ms over 30 days). SLOs are derived from SLAs and drive prioritization — if the SLO is at risk, reliability work takes precedence over features.' },
                { question: 'What does the RED method stand for?', options: ['Reliability, Errors, Downtime', 'Requests, Events, Data', 'Rate, Errors, Duration', 'Resources, Endpoints, DNS'], correct: 2, explanation: 'RED (Rate, Errors, Duration) is a method for monitoring microservices, popularized by Tom Wilkie. Rate = requests/second, Errors = failed requests/second, Duration = latency distribution. It pairs with the USE method (Utilization, Saturation, Errors) for infrastructure.' },
                { question: 'What is OpenTelemetry?', options: ['A Grafana plugin', 'A vendor-neutral standard for telemetry collection', 'An AWS monitoring service', 'A log format'], correct: 1, explanation: 'OpenTelemetry (OTel) is a CNCF project providing APIs, SDKs, and the OTel Collector for capturing traces, metrics, and logs in a vendor-neutral format. It merges OpenCensus and OpenTracing and is now the standard for instrumenting cloud-native applications.' },
                { question: 'What does Alertmanager handle in the Prometheus stack?', options: ['Storing metrics', 'Scraping targets', 'Routing and deduplication of alerts', 'Visualizing dashboards'], correct: 2, explanation: 'Alertmanager receives firing alerts from Prometheus, deduplicates them, groups related alerts, applies inhibition rules, and routes notifications to the correct receiver (PagerDuty, Slack, email). It prevents alert storms by grouping and silencing.' },
                { question: 'What is Loki used for?', options: ['Log aggregation', 'Metrics storage', 'Tracing', 'Container scanning'], correct: 0, explanation: 'Loki is a Grafana Labs log aggregation system inspired by Prometheus. Unlike Elasticsearch, it indexes only log metadata (labels) and stores log content compressed. This makes it cost-efficient and fast to query when used with Grafana.' },
                { question: 'What does MTTR measure?', options: ['Maximum Throughput Rate', 'Mean Time To Replicate', 'Minimum Traffic Requirement', 'Mean Time To Recovery'], correct: 3, explanation: 'Mean Time To Recovery (MTTR) measures the average time from when an incident begins until the service is fully restored. Low MTTR is a sign of mature incident response processes, good observability, and effective on-call practices.' },
                { question: 'What is distributed tracing used for?', options: ['Distributing alerts', 'Tracking a request across multiple services', 'Tracing network packets', 'Distributing logs'], correct: 1, explanation: 'Distributed tracing follows a single request as it flows through multiple microservices by propagating a trace context (trace ID, span ID). Tools like Jaeger, Zipkin, and Tempo visualize the full call tree, showing latency and errors per service hop.' },
                { question: 'What is PromQL?', options: ['Prometheus Query Language', 'A database engine', 'A protocol buffer format', 'A provisioning tool'], correct: 0, explanation: 'PromQL is Prometheus\'s functional query language for selecting and aggregating time-series data. Example: rate(http_requests_total{status="500"}[5m]) computes the per-second error rate over the last 5 minutes. It powers both dashboards and alerting rules.' },
    ...
    "panels": [
      ...
      },
      {
        ...
        },
        {
            ...
            quiz: [
                { question: 'What file format does Ansible use for playbooks?', options: ['JSON', 'XML', 'YAML', 'INI'], correct: 2, explanation: 'Ansible playbooks are written in YAML, a human-readable data serialization format. The INI format is used for static inventory files. Tasks are defined as YAML lists, making playbooks easy to read and diff in version control.' },
                { question: 'What is an Ansible role?', options: ['A user account', 'Reusable collection of tasks', 'A network configuration', 'A container'], correct: 1, explanation: 'A role is a structured, reusable unit of automation with a standard directory layout (tasks/, handlers/, templates/, vars/, defaults/, files/). Roles can be shared via Ansible Galaxy and included in playbooks with a single line.' },
                { question: 'What is Ansible Vault used for?', options: ['Encrypting sensitive data', 'Storage', 'Container management', 'Load balancing'], correct: 0, explanation: 'Ansible Vault encrypts sensitive files and variables (passwords, API keys, certificates) using AES256. Encrypted files can be safely committed to version control. Decrypt at run time with --ask-vault-pass or a vault password file.' },
                { question: 'How does Ansible connect to managed hosts by default?', options: ['HTTP', 'RDP', 'FTP', 'SSH'], correct: 3, explanation: 'Ansible is agentless — it connects to managed hosts over SSH (or WinRM for Windows) and pushes Python code to execute tasks. No daemon needs to be installed on managed nodes, simplifying setup and reducing attack surface.' },
                { question: 'What are Ansible facts?', options: ['Task results', 'Encrypted variables', 'Auto-collected host information', 'Role dependencies'], correct: 2, explanation: 'Facts are variables automatically gathered about managed hosts at the start of a play (OS, CPU, memory, IP addresses, etc.) via the setup module. Use them in conditionals (when: ansible_os_family == "Debian") or templates. Disable with gather_facts: false for speed.' },
                { question: 'What does the --check flag do in ansible-playbook?', options: ['Checks syntax only', 'Runs in dry-run mode without making changes', 'Checks connectivity', 'Validates the vault'], correct: 1, explanation: 'The --check flag (dry-run mode) simulates what a playbook would do without making any actual changes. Modules report what they would change. Use with --diff to see exact file differences. Useful for safe previewing before applying changes to production.' },
                { question: 'What is the purpose of the "when" keyword in a task?', options: ['Run task at a scheduled time', 'Loop over items', 'Define handlers', 'Conditionally run task based on an expression'], correct: 3, explanation: 'The "when" keyword makes a task conditional. It accepts a Jinja2 expression that is evaluated at runtime. Example: when: ansible_distribution == "Ubuntu" skips the task on non-Ubuntu hosts. Multiple conditions can be combined with and/or.' },
                { question: 'What does "become: yes" enable in Ansible?', options: ['Privilege escalation (sudo)', 'Debug mode', 'Fact gathering', 'Check mode'], correct: 0, explanation: '"become: yes" enables privilege escalation, running tasks as a different user (root by default via sudo). Use become_user to specify a different target user. This is needed for tasks that require root privileges like installing packages.' },
                { question: 'What is a dynamic inventory?', options: ['An inventory that updates itself hourly', 'An inventory built at runtime from external sources', 'A YAML inventory file', 'An encrypted inventory'], correct: 1, explanation: 'A dynamic inventory plugin or script queries an external source (AWS EC2, GCP, Azure, Terraform state) at runtime to build the host list. This avoids manually maintaining static inventory files and ensures Ansible always sees the current infrastructure.' },
                { question: 'What replaced Ansible "with_items" in modern versions?', options: ['foreach', 'iterate', 'loop', 'each'], correct: 2, explanation: 'The "loop" keyword replaced "with_items" in Ansible 2.5+. It is simpler and more consistent. "with_items" still works for backwards compatibility, but "loop" is preferred for new playbooks and is compatible with all "with_*" lookup plugins.' },
                { question: 'What templating engine does Ansible use?', options: ['Mustache', 'Handlebars', 'Jinja2', 'ERB'], correct: 2, explanation: 'Ansible uses Jinja2 for all templating, variable substitution ({{ variable }}), and expressions. Jinja2 is used in task parameters, conditionals (when:), and .j2 template files rendered by the template module.' },
                { question: 'What is Ansible AWX?', options: ['A module for AWS', 'An inventory plugin', 'A vault tool', 'Open-source web UI for managing Ansible at scale'], correct: 3, explanation: 'AWX is the open-source upstream project for Red Hat Ansible Automation Platform. It provides a web UI, REST API, RBAC, job scheduling, credentials management, and run logging — making it enterprise-grade for managing Ansible at scale across teams.' },
                { question: 'What does the "rescue" block do in Ansible?', options: ['Runs when tasks in the block fail', 'Saves task results', 'Restores files from backup', 'Escalates privileges'], correct: 0, explanation: 'The "rescue" section inside a "block" runs when any task in the block fails — similar to a try/catch pattern. It\'s used for cleanup, notifications, or recovery steps. An "always" section runs regardless of success or failure.' },
                { question: 'What does the "serial" keyword control in Ansible?', options: ['Playbook order', 'Task execution speed', 'Number of hosts managed at once for rolling updates', 'SSH timeout'], correct: 2, explanation: 'The "serial" keyword controls rolling update behavior by limiting how many hosts are processed simultaneously. serial: 1 updates one host at a time; serial: "25%" updates 25% of the group. This prevents the play from taking down all hosts at once.' },
                { question: 'What does "delegate_to" do in Ansible?', options: ['Delegates RBAC roles', 'Assigns inventory groups', 'Delegates to AWX', 'Runs a task on a different host than the current one'], correct: 3, explanation: '"delegate_to" executes a task on a specified host instead of the current inventory host. Common uses: deregistering a host from a load balancer before patching it, or creating a DNS record on a DNS server for a newly provisioned VM.' }
            ],
            codebase: [
                {
      ...
      vars:
        ...
        },
        {
            ...
            quiz: [
                ...
                { question: 'What is AKS?', options: ['Azure Key Storage', 'Azure Kubernetes Service', 'Azure Knowledge System', 'Azure Kernel Service'], correct: 1, explanation: 'Azure Kubernetes Service (AKS) is a managed Kubernetes offering. Azure handles the control plane (API server, etcd) at no cost; you pay only for worker nodes. It integrates with Azure AD, Azure Monitor, ACR, and Azure networking.' },
                { question: 'What is Azure Blob Storage used for?', options: ['Relational data', 'Unstructured data', 'Real-time streams', 'Container images'], correct: 1, explanation: 'Azure Blob Storage is object storage for unstructured data — files, images, videos, backups, and logs. It offers Hot, Cool, and Archive access tiers. It\'s the equivalent of AWS S3 and is used by many Azure services as a backing store.' },
                { question: 'What is Azure DevOps?', options: ['A programming language', 'Development tools suite', 'A database', 'A container runtime'], correct: 1, explanation: 'Azure DevOps is a suite of developer tools including Boards (work tracking), Repos (Git hosting), Pipelines (CI/CD), Test Plans, and Artifacts (package registry). It competes with GitHub, GitLab, and Jira+Bitbucket.' },
                { question: 'What is Azure Key Vault used for?', options: ['Virtual machine keys', 'Securely storing secrets, certificates, and keys', 'SSH key generation', 'API gateway keys'], correct: 1, explanation: 'Azure Key Vault is a cloud HSM-backed service for securely storing and tightly controlling access to secrets, certificates, and encryption keys. Apps authenticate with Managed Identity to retrieve secrets without hardcoding credentials.' },
                { question: 'What type of load balancer is Azure Load Balancer?', options: ['Layer 7 (HTTP)', 'Layer 4 (TCP/UDP)', 'DNS-based', 'Application Gateway'], correct: 1, explanation: 'Azure Load Balancer operates at Layer 4 (TCP/UDP), distributing inbound flows based on IP address and port. For Layer 7 (HTTP/HTTPS) load balancing with URL routing, SSL termination, and WAF, use Azure Application Gateway instead.' },
                { question: 'What is a Managed Identity in Azure?', options: ['A user account', 'Auto-managed identity for apps to authenticate without credentials', 'A service principal', 'A directory group'], correct: 1, explanation: 'Managed Identity provides Azure services (VMs, Functions, AKS) with an automatically managed identity in Azure AD. Apps can authenticate to any service that supports Azure AD auth (Key Vault, Storage, SQL) without storing credentials in code or config.' },
                { question: 'What is Azure Monitor primarily used for?', options: ['Cost management', 'Full-stack monitoring, metrics, logs, and alerts', 'Container registry', 'Network routing'], correct: 1, explanation: 'Azure Monitor collects metrics and logs from all Azure resources, VMs, and applications. It includes Log Analytics (for log queries using KQL), Application Insights (APM), and Azure Alerts with action groups for notifications and automation.' },
                { question: 'What is Azure Cosmos DB?', options: ['SQL relational database', 'Globally distributed multi-model NoSQL database', 'Data warehouse', 'Cache service'], correct: 1, explanation: 'Azure Cosmos DB is a globally distributed, multi-model (document, key-value, graph, columnar) NoSQL database with guaranteed single-digit millisecond latency. It supports multiple APIs: NoSQL, MongoDB, Cassandra, Gremlin, and Table.' },
                { question: 'What does an ARM template do?', options: ['Manages access roles', 'Defines Azure infrastructure as declarative JSON code', 'Monitors resources', 'Bills resources'], correct: 1, explanation: 'ARM (Azure Resource Manager) templates are JSON files that declaratively define Azure infrastructure. They support parameters, variables, and dependencies. Bicep is a newer, cleaner DSL that compiles to ARM templates and is now the preferred approach.' },
                { question: 'What is Azure Service Bus?', options: ['A network bus', 'Enterprise message broker for decoupling app components', 'A load balancer', 'A service registry'], correct: 1, explanation: 'Azure Service Bus is a fully managed enterprise message broker with queues and publish-subscribe topics. It supports advanced features like sessions, dead-letter queues, scheduled delivery, and transactions — suitable for reliable microservice communication.' },
                { question: 'What is Azure Policy used for?', options: ['CI/CD pipelines', 'Enforcing governance and compliance standards on resources', 'Cost optimization', 'Identity management'], correct: 1, explanation: 'Azure Policy evaluates resources against defined rules (e.g., "all VMs must use approved SKUs" or "storage accounts must require HTTPS"). Non-compliant resources are flagged or remediated. Policies can also deny or audit deployments at creation time.' }
            ],
            codebase: [
                {
      ...
      "value": "[concat('https://', parameters('appName'), '.azurewebsites.net')]"
        ...
        },
        {
            ...
            flashcards: [
                ...
                { term: 'HashiCorp Vault', definition: 'A secrets management tool that securely stores and dynamically generates secrets (API keys, passwords, certificates). Supports secret leasing, renewal, and revocation with detailed audit logs.' },
                { term: 'Threat Modeling', definition: 'A structured process for identifying potential security threats and mitigations early in design. Common frameworks include STRIDE (Spoofing, Tampering, Repudiation, Information Disclosure, DoS, Elevation of Privilege).' },
                { term: 'Defense in Depth', definition: 'A security strategy using multiple layered controls so that if one layer fails, others still protect the system. Combines network firewalls, WAF, RBAC, encryption, monitoring, and patching.' },
                { term: 'Principle of Least Privilege', definition: 'Granting users, services, and processes only the minimum permissions required to perform their function. Reduces blast radius if an account is compromised. Applied to IAM roles, K8s RBAC, and database access.' },
                { term: 'RASP', definition: 'Runtime Application Self-Protection - security technology embedded inside an application that detects and blocks attacks in real time from within the running process, without relying solely on external perimeter defenses.' }
            ],
            commands: [
                { command: 'trivy image myimage:tag', description: 'Scan a container image for OS and library vulnerabilities' },
                { command: 'trivy image --severity CRITICAL,HIGH myimage:tag', description: 'Scan image showing only CRITICAL and HIGH vulnerabilities' },
                { command: 'trivy fs --scanners vuln,secret .', description: 'Scan filesystem for vulnerabilities and secrets' },
                { command: 'trivy config .', description: 'Scan IaC files (Dockerfile, Terraform, K8s) for misconfigurations' },
                { command: 'grype myimage:tag', description: 'Scan container image for vulnerabilities with Anchore Grype' },
                { command: 'syft myimage:tag -o cyclonedx-json', description: 'Generate a CycloneDX SBOM for a container image' },
                { command: 'syft dir:. -o spdx-json > sbom.spdx.json', description: 'Generate an SPDX SBOM for a local directory' },
                { command: 'cosign sign --key cosign.key myimage:tag', description: 'Sign a container image with Sigstore Cosign' },
                { command: 'cosign verify --key cosign.pub myimage:tag', description: 'Verify the signature of a container image' },
                { command: 'snyk test', description: 'Test project dependencies for known vulnerabilities' },
                { command: 'snyk container test myimage:tag', description: 'Test a container image with Snyk' },
                { command: 'snyk iac test main.tf', description: 'Test Terraform IaC files for security issues with Snyk' },
                { command: 'gitleaks detect', description: 'Scan git history for leaked secrets' },
                { command: 'gitleaks detect --source . --no-git', description: 'Scan a directory for secrets without git history' },
                { command: 'semgrep --config=p/owasp-top-ten .', description: 'Run SAST scan with OWASP Top 10 rules' },
                { command: 'semgrep --config=auto .', description: 'Run Semgrep with automatically selected rules for detected languages' },
                { command: 'checkov -f main.tf', description: 'Scan a Terraform file for security misconfigurations' },
                { command: 'checkov -d . --framework terraform,kubernetes', description: 'Scan a directory for Terraform and Kubernetes misconfigurations' },
                { command: 'tfsec .', description: 'Run tfsec security scanner on Terraform code' },
                { command: 'kubesec scan pod.yaml', description: 'Analyze a Kubernetes manifest for security risks' },
                { command: 'kube-bench run --targets node', description: 'Run CIS Kubernetes Benchmark checks on a node' },
                { command: 'opa eval -d policy.rego -i input.json \'data.policy.allow\'', description: 'Evaluate an OPA Rego policy against input data' },
                { command: 'vault secrets list', description: 'List all enabled Vault secrets engines' },
                { command: 'vault kv put secret/myapp password=s3cr3t', description: 'Write a secret to Vault KV store' }
            ],
            quiz: [
                { question: 'What does "Shift Left" mean in DevSecOps?', options: ['Integrate security early', 'Move to cloud', 'Use left-aligned code', 'Deploy faster'], correct: 0, explanation: 'Shift Left means moving security practices earlier in the software development lifecycle — into planning, coding, and CI/CD — rather than only testing at the end. This catches vulnerabilities when they are cheaper and faster to fix.' },
                { question: 'What is SAST?', options: ['Simple Auth Service', 'Server Authentication', 'System Admin Tool', 'Static Application Security Testing'], correct: 3, explanation: 'SAST (Static Application Security Testing) analyzes source code, bytecode, or binaries without running the application. It finds vulnerabilities like SQL injection, XSS, and hardcoded credentials early in the development cycle.' },
                { question: 'What does Trivy scan for?', options: ['Performance issues', 'Container vulnerabilities', 'Network traffic', 'User activity'], correct: 1, explanation: 'Trivy is an open-source vulnerability scanner by Aqua Security. It scans container images, filesystems, and IaC configurations for known CVEs in OS packages and application libraries, as well as misconfigurations and exposed secrets.' },
                { question: 'What is the OWASP Top 10?', options: ['Best frameworks', 'Programming languages', 'Critical web security risks', 'Cloud providers'], correct: 2, explanation: 'The OWASP Top 10 is a standard awareness document published by the Open Worldwide Application Security Project, listing the 10 most critical web application security risks such as Injection, Broken Access Control, and Cryptographic Failures.' },
                { question: 'What is Zero Trust Security?', options: ['No passwords required', 'No user or system is trusted by default', 'No encryption used', 'No cloud access'], correct: 1, explanation: 'Zero Trust is a security model based on "never trust, always verify." Every user, device, and service must authenticate and be authorized for every request, regardless of whether it originates inside or outside the network perimeter.' },
                { question: 'What does SBOM stand for?', options: ['Software Bill of Materials', 'Secure Build Output Map', 'System Binary Object Model', 'Security Baseline Operations Manual'], correct: 0, explanation: 'A Software Bill of Materials (SBOM) is a formal, machine-readable inventory of all software components and their dependencies. It enables organizations to quickly identify which products are affected when a new CVE is disclosed (e.g., Log4Shell).' },
                { question: 'What is Policy as Code?', options: ['Writing documentation', 'Coding security patches', 'A compliance framework', 'Defining security policies in code for automation'], correct: 3, explanation: 'Policy as Code uses tools like Open Policy Agent (OPA) with Rego to express security and compliance rules as version-controlled code. Policies can then be automatically enforced in CI/CD pipelines, admission controllers, and API gateways.' },
                { question: 'What does CSPM stand for?', options: ['Container Security Policy Manager', 'Continuous Security Pipeline Monitor', 'Cloud Security Posture Management', 'Code Scanning and Patch Management'], correct: 2, explanation: 'Cloud Security Posture Management (CSPM) tools continuously monitor cloud infrastructure (AWS, Azure, GCP) for misconfigurations like public S3 buckets, overly permissive IAM roles, and missing encryption, alerting teams to compliance violations.' },
                { question: 'What tool does Sigstore Cosign help with?', options: ['Container networking', 'Signing container images', 'Scanning for CVEs', 'Managing secrets'], correct: 1, explanation: 'Cosign is part of the Sigstore project and enables cryptographic signing and verification of container images. Signing images ensures they have not been tampered with since build time and that they come from a trusted source.' },
                { question: 'What is Falco used for?', options: ['Runtime security monitoring', 'IaC scanning', 'Container building', 'Secret rotation'], correct: 0, explanation: 'Falco is a cloud-native runtime security tool by Sysdig/CNCF that monitors Linux system calls to detect anomalous behavior in containers and Kubernetes workloads, such as shell spawning inside a container or unexpected file access.' },
                { question: 'What does secrets rotation protect against?', options: ['Source code leaks', 'Network attacks', 'Dependency vulnerabilities', 'Impact of credential compromise'], correct: 3, explanation: 'Secrets rotation limits the window of exposure when credentials are compromised. By regularly cycling API keys, passwords, and certificates, even if a secret is leaked, it becomes invalid before an attacker can make significant use of it.' },
                { question: 'What is IAST?', options: ['A firewall type', 'A cloud security scanner', 'Interactive Application Security Testing combining SAST and DAST', 'An IaC tool'], correct: 2, explanation: 'IAST (Interactive Application Security Testing) instruments the running application from within during functional testing. By observing the application internally, it combines the accuracy of SAST with the real-execution coverage of DAST, producing fewer false positives.' },
                { question: 'What does SLSA help protect against?', options: ['DDoS attacks', 'Software supply chain tampering', 'Container escape', 'Network intrusion'], correct: 1, explanation: 'SLSA (Supply chain Levels for Software Artifacts) is a security framework providing incremental levels of assurance against supply chain attacks. It ensures builds are reproducible, scripts and binaries are tracked, and provenance is verifiable.' },
                { question: 'What is OWASP ZAP primarily used for?', options: ['Dynamic application security testing', 'Static code analysis', 'Container scanning', 'Secret rotation'], correct: 0, explanation: 'OWASP ZAP (Zed Attack Proxy) is a free, open-source DAST tool. It proxies HTTP traffic and actively probes running web applications for vulnerabilities like XSS, CSRF, and injection flaws. It supports both automated and manual testing modes.' },
                { question: 'What does SonarQube analyze?', options: ['Container images', 'Network traffic', 'Code quality and security vulnerabilities', 'Cloud misconfigurations'], correct: 2, explanation: 'SonarQube performs static analysis on source code to detect bugs, code smells, and security vulnerabilities (called hotspots). It integrates into CI/CD pipelines and provides quality gates that can block merges when thresholds are not met.' }
            ],
            codebase: [
                {
...
ssl_stapling_verify on;`
        ...
        },
        {
            ...
            flashcards: [
                ...
                { term: 'TCP Three-Way Handshake', definition: 'The process of establishing a TCP connection: SYN (client initiates), SYN-ACK (server acknowledges), ACK (client confirms). Ensures both sides are ready before data transfer.' },
                { term: 'DHCP', definition: 'Dynamic Host Configuration Protocol - Automatically assigns IP addresses, subnet masks, gateways, and DNS servers to devices joining a network, eliminating manual configuration.' },
                { term: 'ICMP', definition: 'Internet Control Message Protocol - Used for network diagnostics and error reporting. Powers ping (echo request/reply) and traceroute. Operates at Layer 3 without ports.' },
                { term: 'IPv6', definition: 'Internet Protocol version 6 - Uses 128-bit addresses (e.g., 2001:db8::1) providing ~340 undecillion unique addresses, solving IPv4 exhaustion. Includes built-in IPsec support.' },
                { term: 'Network Namespace', definition: 'A Linux kernel feature that provides isolated network stacks (interfaces, routing tables, iptables rules) per process group. Used by Docker and Kubernetes to isolate container networking.' },
                { term: 'VXLAN', definition: 'Virtual Extensible LAN - An overlay network protocol that encapsulates Layer 2 frames in UDP packets, enabling large-scale multi-tenant cloud networks across Layer 3 boundaries. Used by Docker, Kubernetes, and cloud providers.' },
                { term: 'Anycast', definition: 'A routing method where multiple servers share the same IP address and traffic is routed to the topographically nearest one. Used by CDNs (Cloudflare) and root DNS servers for low-latency, resilient delivery.' },
                { term: 'East-West Traffic', definition: 'Network traffic flowing laterally between services within a data center or cluster. Increasing with microservices architectures and often secured with mTLS via a service mesh.' },
                { term: 'North-South Traffic', definition: 'Network traffic flowing between external clients and internal services, crossing the network perimeter. Secured by load balancers, firewalls, WAFs, and ingress controllers.' },
                { term: 'eBPF', definition: 'Extended Berkeley Packet Filter - A Linux kernel technology allowing safe custom programs to run in kernel space for high-performance networking, observability, and security. Powers tools like Cilium and Falco.' },
                { term: 'Service Discovery', definition: 'A mechanism enabling services to dynamically find each other without hardcoded IPs. Client-side (Consul, Eureka) or server-side (Kubernetes DNS, AWS ELB) patterns are common.' },
                { term: 'SNI', definition: 'Server Name Indication - A TLS extension where the client specifies the target hostname during the handshake, enabling a single server/IP to serve multiple SSL/TLS certificates (virtual hosting over HTTPS).' },
                { term: 'Well-Known Ports', definition: 'Standardized TCP/UDP ports: 22 (SSH), 25 (SMTP), 53 (DNS), 80 (HTTP), 443 (HTTPS), 3306 (MySQL), 5432 (PostgreSQL), 6379 (Redis), 27017 (MongoDB).' },
                { term: 'QUIC / HTTP/3', definition: 'QUIC is a transport protocol built on UDP developed by Google, providing TLS 1.3, multiplexing, and 0-RTT connection resumption. HTTP/3 uses QUIC as its transport layer, eliminating TCP head-of-line blocking.' },
                { term: 'Rate Limiting', definition: 'Controlling the frequency of requests from a client to protect services from abuse, DDoS, or overload. Implemented at the API gateway, load balancer, or application layer using algorithms like token bucket or leaky bucket.' },
                { term: 'Health Check', definition: 'An automated probe (HTTP, TCP, or command) that determines if a service instance is alive and ready to receive traffic. Load balancers and orchestrators use health checks to route away from unhealthy instances.' }
            ],
            commands: [
            ...
            quiz: [
                { question: 'What does DNS translate?', options: ['IP to MAC', 'Domain names to IPs', 'Ports to services', 'HTTP to HTTPS'], correct: 1, explanation: 'DNS (Domain Name System) resolves human-readable domain names like example.com into IP addresses that computers use to route traffic.' },
                { question: 'What is a load balancer used for?', options: ['Encryption', 'Distributing traffic', 'DNS resolution', 'File storage'], correct: 1, explanation: 'A load balancer distributes incoming requests across multiple backend servers to improve availability and prevent any single server from being overwhelmed.' },
                { question: 'What does CIDR stand for?', options: ['Cloud Internet Domain Routing', 'Classless Inter-Domain Routing', 'Central IP Data Registry', 'Cluster IP Distribution Rule'], correct: 1, explanation: 'CIDR (Classless Inter-Domain Routing) is a method for allocating IP addresses using prefix notation (e.g., 10.0.0.0/16) to define network size without fixed class boundaries.' },
                { question: 'What does NAT do?', options: ['Encrypts traffic', 'Maps private to public IPs', 'Balances load', 'Resolves DNS'], correct: 1, explanation: 'NAT (Network Address Translation) maps private IP addresses to a public IP, allowing multiple devices on a private network to share a single public IP address.' },
                { question: 'How many layers does the OSI model have?', options: ['4', '5', '6', '7'], correct: 3, explanation: 'The OSI model has 7 layers: Physical, Data Link, Network, Transport, Session, Presentation, and Application. Each layer handles a specific aspect of network communication.' },
                { question: 'What transport protocol does DNS primarily use?', options: ['TCP', 'UDP', 'HTTP', 'ICMP'], correct: 1, explanation: 'DNS uses UDP on port 53 for most queries because it is faster and DNS responses are small. TCP is used for large responses (over 512 bytes) or zone transfers.' },
                { question: 'What does mTLS add over standard TLS?', options: ['Faster encryption', 'Client certificate authentication', 'Compression', 'UDP support'], correct: 1, explanation: 'Mutual TLS (mTLS) requires both the client and server to present certificates, enabling two-way authentication. Standard TLS only authenticates the server.' },
                { question: 'What protocol does gRPC use for transport?', options: ['HTTP/1.1', 'HTTP/2', 'WebSocket', 'TCP raw'], correct: 1, explanation: 'gRPC is built on HTTP/2, which enables bidirectional streaming, multiplexed requests over a single connection, and efficient binary framing via Protocol Buffers.' },
                { question: 'What does ARP resolve?', options: ['Domain names to IPs', 'IP addresses to MAC addresses', 'Ports to services', 'IPs to hostnames'], correct: 1, explanation: 'ARP (Address Resolution Protocol) maps an IP address to the corresponding MAC (hardware) address on a local network segment so frames can be delivered correctly.' },
                { question: 'What is a CDN primarily used for?', options: ['Database replication', 'Reducing latency by serving from nearby servers', 'Container orchestration', 'Log aggregation'], correct: 1, explanation: 'A CDN (Content Delivery Network) caches static assets at geographically distributed edge nodes, serving requests from the nearest location to reduce latency and origin load.' },
                { question: 'What is the default MTU size for Ethernet?', options: ['512 bytes', '1024 bytes', '1500 bytes', '9000 bytes'], correct: 2, explanation: 'The standard Ethernet MTU (Maximum Transmission Unit) is 1500 bytes. Jumbo frames use 9000 bytes but require explicit configuration and end-to-end support.' },
                { question: 'What does a VLAN do?', options: ['Encrypts traffic', 'Logically segments a physical network', 'Resolves DNS', 'Balances load'], correct: 1, explanation: 'A VLAN (Virtual LAN) uses tags (IEEE 802.1Q) to segment a physical switch into isolated broadcast domains, improving security and reducing unnecessary broadcast traffic.' },
                { question: 'What is the difference between a forward proxy and a reverse proxy?', options: ['No difference', 'Forward proxy serves clients; reverse proxy serves backend servers', 'Reverse proxy is for databases only', 'Forward proxy only works with HTTPS'], correct: 1, explanation: 'A forward proxy sits in front of clients and forwards their requests to the internet (e.g., for filtering or anonymity). A reverse proxy sits in front of servers and routes inbound client traffic to backends.' },
                { question: 'What key feature does HTTP/2 add over HTTP/1.1?', options: ['Encryption', 'Multiplexing multiple requests over one connection', 'WebSocket support', 'Larger cookies'], correct: 1, explanation: 'HTTP/2 introduces multiplexing, allowing multiple requests and responses to be in flight simultaneously over a single TCP connection, eliminating head-of-line blocking from HTTP/1.1.' },
                { question: 'Which BGP role manages routing between autonomous systems?', options: ['OSPF', 'eBGP', 'RIP', 'EIGRP'], correct: 1, explanation: 'eBGP (External BGP) is used to exchange routing information between different autonomous systems (e.g., between ISPs or cloud providers). iBGP handles routing within a single AS.' }
            ],
            codebase: [
                {
