...
const QUESTIONS = {
    docker: [
        ...
        { q: "What does the -d flag do in 'docker run -d'?", a: ["Runs container in detached mode", "Enables debug logging", "Runs container with elevated privileges", "Disables network isolation"], c: 0, e: "Detached mode returns the container ID and runs the container in the background; use docker logs <id> to view output." },
        ...
        { q: "What does the HEALTHCHECK Dockerfile instruction do?", a: ["Tells Docker how to test if the container is healthy", "Installs a health monitoring agent", "Exposes a health endpoint on port 8080", "Validates the image before it is pushed"], c: 0, e: "HEALTHCHECK runs a command (e.g., curl /health) at intervals; the container status becomes 'healthy' or 'unhealthy' — orchestrators use this to restart or drain unhealthy instances." },
        { q: "What is the purpose of a Docker volume?", a: ["Persist data independently of the container lifecycle", "Speed up container startup time", "Share environment variables between containers", "Reduce the image layer count"], c: 0, e: "Volumes survive container removal and can be shared across containers; bind mounts map a host path, named volumes are managed by Docker." }
    ],
    kubernetes: [
        ...
        { q: "What is the difference between resource 'requests' and 'limits' in a Kubernetes Pod spec?", a: ["Requests are used for scheduling; limits are the maximum the container can consume", "Limits are used for scheduling; requests cap the container's usage", "Requests and limits both set the maximum resource usage", "Requests are optional; limits are mandatory for all pods"], c: 0, e: "The scheduler places pods on nodes that have enough capacity to satisfy requests; limits prevent a container from starving neighbors. Exceeding CPU limit causes throttling; exceeding memory limit causes OOMKill." },
        { q: "What is a Kubernetes ConfigMap?", a: ["Stores non-sensitive configuration data as key-value pairs", "Stores encrypted passwords and tokens", "Defines resource limits for pods", "Manages network routing rules"], c: 0, e: "ConfigMaps decouple configuration from container images; mount them as environment variables or files. Use Secrets for sensitive values." }
    ],
    cicd: [
        ...
        { q: "What triggers a CI pipeline?", a: ["Code commit/push", "Container image pull from a registry", "Application health check passing", "Database schema migration completing"], c: 0, e: "Pipelines are event-driven: push, pull request, tag, or schedule events trigger the pipeline via webhooks." },
        ...
        { q: "What is the primary benefit of caching dependencies in a CI pipeline?", a: ["Avoids re-downloading packages on every run, reducing build time", "Ensures builds always use the latest package versions", "Prevents dependency version conflicts between jobs", "Encrypts package downloads for security"], c: 0, e: "Cache keys are usually a hash of the lockfile (package-lock.json, Pipfile.lock); when the lockfile changes the cache is invalidated and rebuilt." }
    ],
    aws: [
    ...
    linux: [
        ...
        { q: "Which command streams live systemd journal logs for a specific service?", a: ["journalctl -u nginx -f", "tail -f /var/log/nginx.log", "systemctl logs nginx --live", "dmesg -u nginx"], c: 0, e: "journalctl -u <unit> -f follows new log entries in real time; add --since '10 min ago' to limit history. journalctl -xe shows the most recent entries with context." }
    ],
    monitoring: [
    ...
    devsecops: [
        ...
        { q: "What is SLSA (Supply-chain Levels for Software Artifacts)?", a: ["A framework for securing the software build and supply chain", "A static application security testing tool", "A container image vulnerability scanner", "A secret rotation protocol"], c: 0, e: "SLSA defines four levels of supply chain integrity; higher levels require signed build provenance proving exactly how and where an artifact was produced." },
        ...
        { q: "What is container scanning?", a: ["Checking images for vulnerabilities in OS packages and app libraries", "Profiling container CPU and memory usage at runtime", "Encrypting network traffic between containers", "Auditing container filesystem access in production"], c: 0, e: "Tools like Trivy and Grype scan image layers for CVEs in OS packages and app libraries; run in CI before pushing to registry." },
        { q: "What is secret management?", a: ["Storing and retrieving sensitive credentials securely at runtime", "Encrypting all data at rest in a database", "Scanning source code for hardcoded API keys", "Rotating TLS certificates on a schedule"], c: 0, e: "Never hardcode secrets; use Vault, AWS Secrets Manager, or Azure Key Vault and retrieve them at runtime via managed identity." },
