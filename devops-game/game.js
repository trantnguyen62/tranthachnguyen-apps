...
const QUESTIONS = {
    docker: [
        ...
        { q: "What does 'docker system prune' remove?", a: ["Stopped containers, unused networks, dangling images, and build cache", "All images including those used by running containers", "Only running containers and their volumes", "Only dangling images and build cache"], c: 0 },
        ...
        { q: "What is the main benefit of a .dockerignore file?", a: ["Reduces build context size and keeps sensitive files out of the image", "Speeds up container runtime startup", "Prevents containers from accessing the host filesystem", "Limits the number of image layers created"], c: 0 },
        ...
        { q: "What Dockerfile instruction declares a build-time variable overridable with --build-arg?", a: ["ARG", "ENV", "VAR", "BUILD"], c: 0 },
        { q: "Which command shows live CPU, memory, and network stats for running containers?", a: ["docker stats", "docker top", "docker inspect", "docker metrics"], c: 0 },
    ...
    kubernetes: [
        { q: "Two containers are in the same Pod. What do they share by default?", a: ["Network namespace and localhost", "Persistent volume claims", "Environment variables", "CPU and memory limits"], c: 0 },
        ...
        { q: "Which command rolls back a Deployment to its previous revision?", a: ["kubectl rollout undo deployment/<name>", "kubectl rollback deployment/<name>", "kubectl apply --previous deployment/<name>", "kubectl revert deployment/<name>"], c: 0 },
        { q: "What is a CronJob?", a: ["Scheduled job", "Continuous job", "One-time job", "Parallel job"], c: 0 },
        { q: "What does a Kubernetes NetworkPolicy resource do?", a: ["Controls which pods can send and receive traffic from other pods", "Assigns IP addresses to pods", "Routes external traffic into the cluster", "Limits CPU and memory for pods"], c: 0 }
    ],
    cicd: [
        ...
        { q: "In GitHub Actions, what does the 'needs' key in a job definition specify?", a: ["Jobs that must complete successfully before this job runs", "Required secrets for the job", "Minimum runner version needed", "Required file dependencies"], c: 0 },
        ...
        { q: "What is the purpose of a 'quality gate' in a CI/CD pipeline?", a: ["Blocks promotion if code quality or test coverage thresholds are not met", "Controls who can merge pull requests", "Sets the maximum allowed pipeline run time", "Defines which environment to deploy to next"], c: 0 },
        { q: "What is ArgoCD?", a: ["GitOps continuous delivery tool", "Source code repository", "Container registry", "Monitoring platform"], c: 0 },
        { q: "What is the main advantage of promoting immutable build artifacts through pipeline stages?", a: ["The exact same binary tested in staging is deployed to production", "Builds run faster on each stage", "Secrets are baked in securely at build time", "Rollbacks require no additional pipeline run"], c: 0 },
    ...
    aws: [
        { q: "Which EC2 pricing model offers the biggest discount in exchange for a 1- or 3-year commitment?", a: ["Reserved Instances", "Spot Instances", "On-Demand Instances", "Dedicated Hosts"], c: 0 },
        { q: "What is S3 used for?", a: ["Object storage", "Compute instances", "Database hosting", "Network routing"], c: 0 },
        { q: "What is the maximum execution timeout for an AWS Lambda function?", a: ["15 minutes", "5 minutes", "30 minutes", "1 hour"], c: 0 },
        ...
        { q: "What is the purpose of CloudWatch Logs Insights?", a: ["Interactively query and analyze log data using a purpose-built query language", "Archive logs to S3 automatically", "Forward logs to third-party SIEM tools", "Set up metric alarms triggered by log patterns"], c: 0 },
        ...
        { q: "What is the key difference between SNS and SQS?", a: ["SNS is push-based pub/sub fanout; SQS is a pull-based message queue", "SQS supports multiple subscribers; SNS does not", "SNS persists messages indefinitely; SQS deletes them immediately", "SQS delivers to all subscribers at once; SNS delivers to one"], c: 0 },
    ...
    terraform: [
        ...
        { q: "What does 'terraform fmt' do?", a: ["Rewrites Terraform config files to canonical HCL style", "Validates config syntax for errors", "Formats the state file for readability", "Pretty-prints the terraform plan output"], c: 0 },
        ...
        { q: "What happens when two engineers run 'terraform apply' simultaneously on a remote backend with state locking?", a: ["The second apply is blocked until the first releases the lock", "Both applies run in parallel and merge their changes", "The second apply overwrites the first apply's state", "Terraform randomly selects which apply proceeds"], c: 0 },
        ...
        { q: "What advantage does 'for_each' have over 'count' when creating multiple resource instances?", a: ["Resources get stable identities from map keys, preventing unwanted replacements", "for_each creates resources faster than count", "count cannot be used with modules", "for_each supports conditional creation; count does not"], c: 0 },
    ...
    git: [
        ...
        { q: "What does 'git reflog' let you recover?", a: ["Commits no longer referenced by any branch or tag", "Deleted remote branches", "Uncommitted file changes", "Stashed changes that expired"], c: 0 },
        { q: "What does 'git log --oneline --graph --all' display?", a: ["Visual ASCII branch history for all branches in one-line format", "All commits across all remote repositories", "Only commits reachable from the current branch", "Commits grouped by author name"], c: 0 },
        { q: "What does 'git diff HEAD~1 HEAD' show?", a: ["Changes introduced by the most recent commit", "Differences between two remote branches", "Unstaged working-directory changes", "Changes between the index and working tree"], c: 0 },
    ...
    linux: [
        ...
        { q: "What is the difference between 'kill -15' and 'kill -9'?", a: ["-15 sends SIGTERM (graceful); -9 sends SIGKILL (immediate, cannot be caught)", "-15 kills a process group; -9 kills only one process", "-9 sends SIGTERM; -15 sends SIGKILL", "-15 requires root privileges; -9 does not"], c: 0 },
        { q: "What is systemd?", a: ["System and service manager", "File system", "Network manager", "Package manager"], c: 0 },
        { q: "Which command shows real-time kernel ring buffer messages, useful for debugging boot or hardware issues?", a: ["dmesg", "journalctl -xe", "syslog -f", "top -b"], c: 0 },
        ...
        { q: "What does 'tar -xzvf archive.tar.gz' do?", a: ["Extracts a gzip-compressed tar archive with verbose output", "Creates a compressed archive from a directory", "Lists archive contents without extracting", "Compresses only changed files into a new archive"], c: 0 },
    ...
    monitoring: [
        { q: "What collection model does Prometheus use to gather metrics from targets?", a: ["Pull-based: Prometheus scrapes HTTP endpoints on targets", "Push-based: targets send metrics to Prometheus", "Agent-based: a sidecar forwards metrics to central storage", "Event-driven: targets emit metrics only on state change"], c: 0 },
        { q: "What is Grafana used for?", a: ["Data visualization", "Data storage", "Data processing", "Data backup"], c: 0 },
        { q: "What is the 'USE Method' for performance analysis?", a: ["Utilization, Saturation, and Errors for every resource", "Usage, Speed, and Efficiency per service", "Uptime, Stability, and Errors per host", "Utilization, Speed, and Events per component"], c: 0 },
        { q: "In Prometheus, what does Alertmanager handle after a rule fires?", a: ["Deduplication, grouping, silencing, and routing alerts to receivers", "Storing alert history in a time-series database", "Evaluating alerting rules against live metrics", "Scraping endpoints to collect new metric samples"], c: 0 },
    ...
    azure: [
        ...
        { q: "What is the purpose of Azure Managed Identities?", a: ["Allow Azure services to authenticate to other services without storing credentials", "Manage user passwords in Azure AD", "Provision new Azure subscriptions automatically", "Monitor identity-related security events"], c: 0 },
        ...
        { q: "Which Azure service provides a Web Application Firewall (WAF) to protect against common web exploits?", a: ["Azure Application Gateway or Azure Front Door with WAF policy", "Azure Firewall", "Network Security Group (NSG)", "Azure DDoS Protection Standard"], c: 0 },
    ...
    networking: [
        ...
        { q: "What is the key difference between TCP and UDP?", a: ["TCP guarantees delivery and ordering; UDP is connectionless and faster but unreliable", "UDP encrypts all traffic; TCP does not", "TCP works at layer 7; UDP works at layer 3", "UDP supports more concurrent connections than TCP"], c: 0 },
        { q: "What does HTTP status code 502 Bad Gateway mean?", a: ["The server acting as a gateway received an invalid response from the upstream server", "The client sent a malformed request", "The requested resource was permanently moved", "The server is temporarily unavailable due to maintenance"], c: 0 },
        ...
        { q: "What is mutual TLS (mTLS)?", a: ["Both the client and server authenticate each other using certificates", "TLS where only the server provides a certificate", "A TLS version that requires two separate handshakes", "TLS with automatic key rotation every session"], c: 0 },
